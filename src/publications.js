import React from "react";

export default function Publications() {
    return (
        <div className="infoBoxes">
            <div className="box">
                <div>
                    <img alt="" src="Books/Starke-Marken.jpg" />
                    <div className="textBorder">
                        <p id="text1">
                            <strong>
                                Starke Marken – systemische Entwicklung und
                                Führung, ganzheitliches Beziehungsmanagement
                            </strong>
                            <br />
                            Mit einem Vorwort von Prof. Dr. Franz-Rudolf
                            Esch,EBS Business School
                            <br />
                            <em>
                                Verlag Unternehmer Medien GmbH, Bonn, September
                                2013
                            </em>
                        </p>
                    </div>
                </div>
            </div>
            <div className="box">
                <div>
                    <img alt="" src="Books/buch3.jpg" />
                    <div className="textBorder">
                        <p id="text2">
                            <strong>
                                Gestalt im Management – Eine andere Sicht auf
                                Marken- und Unternehmensführung in komplexen
                                Märkten
                            </strong>
                            <br />
                            <em>Springer Gabler Verlag, Wiesbaden, 2013</em>
                        </p>
                    </div>
                </div>
            </div>
            <div className="box">
                <div>
                    <img alt="" src="Books/Kanzlei.jpg" />
                    <div className="textBorder">
                        <p id="text3">
                            <strong>
                                Salomo Friedlaender/Mynona - Impulsgeber der
                                Gestaltherapie / Balance, Nullpunkt,
                                Schöpferische Indifferenz
                            </strong>
                            <br />
                            Titel des eigenen Beitrags: „Einfach beim Wort
                            nehmen“
                            <em>
                                Herausgeber Detlef Thiel / Kohlhage Verlag,
                                Bergisch Gladbach, 2014
                            </em>
                            <br />
                        </p>
                    </div>
                </div>
            </div>
            <div className="box">
                <div>
                    <img alt="" src="Books/buch2.jpg" />
                    <div className="textBorder">
                        <p id="text4">
                            <strong>
                                Warum funktionieren Aufstellungen? Eine
                                Betrachtung in 14 Thesen
                            </strong>
                            <br />
                            <em>
                                Deutscher Wissenschafts-Verlag, Baden Baden,
                                2014 (erscheint in englischer Sprache in 2018)
                            </em>
                        </p>
                    </div>
                </div>
            </div>
            <div className="box">
                <div>
                    <img alt="" src="Books/Friedlaender-Gestalt.jpg" />
                    <div className="textBorder">
                        <p id="text5">
                            <strong>Die Kanzlei als erfolgreiche Marke</strong>
                            <br />
                            Mit Vorwörtern von Markus Hartung, Direktor an der
                            Bucerius Law School, <br />
                            Hamburg und Prof. Dr. Leo Staub, Direktor
                            Universität St. Gallen
                            <br />
                            <em>Springer Gabler Verlag, Wiesbaden, 2015</em>
                        </p>
                    </div>
                </div>
            </div>
            <div className="box">
                <div>
                    <img alt="" src="Books/buch1.jpg" />
                    <div className="textBorder">
                        <p id="text6">
                            <strong>
                                Theorie und Praxis der Organisationsaufstellung–
                                Grundlagen für systemische Personal- und
                                Organisationsentwicklung
                            </strong>
                            <br />
                            <em>
                                Springer Gabler Verlag, Wiesbaden, März 2018
                            </em>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    );
}
